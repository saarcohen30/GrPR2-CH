...
class MADDPG(MARLAlgorithm):
    ...
    def _create_placeholders(self):
        ...
        if self.opponent_modelling:
            self._recent_opponent_observations_ph = tf.compat.v1.placeholder(
                tf.float32,shape=[None, self._observation_dim],
                name='recent_opponent_observations_agent_{}'.format(self._agent_id))
            self._recent_opponent_actions_pl = tf.compat.v1.placeholder(
                tf.float32, shape=[None, self._opponent_action_dim],
    ...
    def _create_q_update(self):
        ...
        with tf.compat.v1.variable_scope('target_q_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
            ...
            q_value_targets = self.target_qf.output_for(
                ...
                actions=next_actions, reuse=tf.compat.v1.AUTO_REUSE)
        ...
        if not self.SGA:
            with tf.compat.v1.variable_scope('q_opt_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
                if self._train_qf:
                    q_train_op = tf.compat.v1.train.AdamOptimizer(self._qf_lr).minimize(
    ...
    def _create_opponent_p_update(self):
        ...
        with tf.compat.v1.variable_scope('opponent_policy_opt_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
    ...
    def _create_p_update(self):
        ...
        q_targets = self.qf.output_for(self._observations_ph, actions, reuse=tf.compat.v1.AUTO_REUSE)  # N
