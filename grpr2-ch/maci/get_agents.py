...
from maci.learners import REGMAAC, MADDPG
...
from maci.value_functions.attention_critic import AttentionCritic
...
from maci.misc.soft_utils import soft_update, hard_update
...
def get_level_k_policy(env, k, M, agent_id, u_range, opponent_conditional_policy, game_name='matrix'):
    ...
    k_policy = MultiLevelPolicy(env_spec=env.env_specs,
                                ...
                                agent_id=agent_id)
    with tf.compat.v1.variable_scope('target_levelk_{}'.format(agent_id), reuse=True):
...
def regma_ac_agent(model_name, agent_num, i, env, M, u_range, base_kwargs, k=0, g=False, mu=1.5, game_name='matrix', aux=True, attend_heads=4):
    joint = False
    # squash = True
    # squash_func = tf.tanh
    # correct_tanh = True
    ...
    pool = SimpleReplayBuffer(env.env_specs, agent_num, max_replay_buffer_size=1e4, joint=joint, agent_id=i)
    ...
    # sa_size = []
    # for acsp, obsp in zip(env.env_spec.action_space,
    #                       env.env_spec.observation_space):
    ...
    # joint_qf = AttentionCritic(sa_size, hidden_dim=M, attend_heads=attend_heads)
    # target_joint_qf = AttentionCritic(sa_size, hidden_dim=M, attend_heads=attend_heads)
    # hard_update(joint_qf, target_joint_qf)
    # self.critic_optimizer = Adam(joint_qf.parameters(), lr=0.01, weight_decay=1e-3)
...
def ddpg_agent(joint, agent_num, opponent_modelling, model_name, i, env, M, u_range, base_kwargs, game_name='matrix'):
    # joint = True
