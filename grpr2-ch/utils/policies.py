...
class DiscreteConditionalPolicy(BasePolicy):
    """
    Policy Network for discrete action spaces
    """
    def __init__(self, cond_policy, *args, **kwargs):
        super(DiscreteConditionalPolicy, self).__init__(*args, **kwargs)
        self.cond_policy = cond_policy
        self.obs_dim = self.cond_policy.input_dim
        
    def forward(self, obs_message, opponent_actions=None, sample=True, return_all_probs=False,
                return_log_pi=False, regularize=False,
                return_entropy=False, return_hidden_state=True):

        opponent_rets = self.cond_policy.forward(obs_message, sample=sample, return_all_probs=return_all_probs,
                return_log_pi=return_log_pi, regularize=regularize,
                return_entropy=return_entropy, return_hidden_state=return_hidden_state)

        obs     = obs_message[:,:self.obs_dim]
        message = obs_message[:,self.obs_dim:]

        if opponent_actions is not None:
            obs_action = torch.cat((obs, opponent_actions), dim=1)
        else:   
            obs_action = torch.cat((obs, opponent_rets[0]), dim=1)
        
        out, hidden_state = super(DiscreteConditionalPolicy, self).forward(obs_action, message)
        probs = F.softmax(out, dim=1)
        on_gpu = next(self.parameters()).is_cuda
        if sample:
            int_act, act = categorical_sample(probs, use_cuda=on_gpu)
