"""
This code constitutes the code required for executing
the desired experiments. For the sake of its implementation, 
we drew inspiration from the code implementations fo the 
following papers with specific emphasis on the first two papers:

[1] Iqbal, Shariq, and Fei Sha. "Actor-attention-critic for multi-agent reinforcement learning." In International Conference on Machine Learning, pp. 2961-2970. PMLR, 2019.

[2] Tian, Z., Wen, Y., Gong, Z., Punakkath, F., Zou, S., & Wang, J. (2019, August). A regularized opponent model with maximum entropy objective. In IJCAI International Joint Conference on Artificial Intelligence (Vol. 28, pp. 602-608). International Joint Conferences on Artifical Intelligence (IJCAI).

[3] Du, Y., Liu, B., Moens, V., Liu, Z., Ren, Z., Wang, J., Chen, X. and Zhang, H., 2021, May. Learning Correlated Communication Topology in Multi-Agent Reinforcement learning. In Proceedings of the 20th International Conference on Autonomous Agents and MultiAgent Systems (pp. 456-464).


However, the original codes were not suitable for our own context, 
and were thus vastly altered in a manner that it would be capable 
of accounting for graph probabilistic recursive reasoning (GrPR2).
Specifically, [1,2] are restricted to fully-observable settings, 
whereas Du et al. [3] assume each agent's policy is learned 
independently of the other agents' policies. 
Additionally, Du et al. [3] do not account for the optimality of
each agent's policy, which we approximate its likelihood via 
the variational lower bound of optimalit presented in the full paper.
"""
import argparse
import torch
import os
...
from algorithms.attention_regma_ac import AttentionREGMASAC
import time
import datetime
...
def run(config):
    ...
    model = AttentionREGMASAC.init_from_env(env,
                                       tau=config.tau,
                                       pi_lr=config.pi_lr,
                                       q_lr=config.q_lr,
                                       gamma=config.gamma,
    ...
    suffix = '{}/{}/{}/{}'.format(config.model_name, len(agent_ob_list), 'REGMA', timestamp)

    print(suffix)

    csv_logger.add_tabular_output('./log/{}.csv'.format(suffix))
    ...
    for ep_i in range(0, config.n_episodes, config.n_rollout_threads):
        ...
        if (ep_i) % config.test_interval == 0:
            for test_ep_i in range(10):
                ...
                agent_comm = [0 for i in range(len(agent_ob_list))]
        ...
        path_length = 0
        path_return = [np.array([0.] * len(agent_ob_list), dtype=np.float32) for i in range(config.n_rollout_threads)]
        last_path_return = [np.array([0.] * len(agent_ob_list), dtype=np.float32) for i in range(config.n_rollout_threads)]
        max_path_return = [np.array([-np.inf] * len(agent_ob_list), dtype=np.float32) for i in range(config.n_rollout_threads)]
        ...
        for et_i in range(config.episode_length):
            ...
            path_length += 1
            path_return += np.array(rewards, dtype=np.float32)
            ...
            if path_length >= 30:
                obs = env.reset()
                max_path_return = np.maximum(max_path_return, path_return)
                mean_path_return = path_return / path_length
                last_path_return = path_return
                for j in range(config.n_rollout_threads):
                  for i in range(len(agent_ob_list)):
                      csv_logger.record_tabular('max-path-return_agent_{}_rollout_{}'.format(i,j), max_path_return[j][i])
                      csv_logger.record_tabular('mean-path-return_agent_{}_rollout_{}'.format(i,j), mean_path_return[j][i])
                      csv_logger.record_tabular('last-path-return_agent_{}_rollout_{}'.format(i,j), last_path_return[j][i])
        ...
        for a_i, a_ep_rew in enumerate(ep_rews):
            csv_logger.record_tabular('mean-episode-rewards_agent_{}'.format(a_i), a_ep_rew)
            logger.add_scalar('agent%i/mean_episode_rewards' % a_i,
                              a_ep_rew * config.episode_length, ep_i)
        
        csv_logger.record_tabular('mean-episode-global-rewards', np.mean(np.array(ep_rews)))
        csv_logger.record_tabular('std-episode-global-rewards', np.std(np.array(ep_rews)))
        csv_logger.dump_tabular(with_prefix=False)
