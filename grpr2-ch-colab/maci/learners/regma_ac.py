...
class REGMAAC(MARLAlgorithm):
    def __init__(
        ...
        super(REGMAAC, self).__init__(**base_kwargs)
    ...
    def _create_q_update(self):
        ...
        with tf.compat.v1.variable_scope('target_joint_q_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
            if self.opponent_action_range is None:
                opponent_target_actions = tf.compat.v1.random_uniform(
        ...
        with tf.compat.v1.variable_scope('target_joint_qf_opt_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
            if self._train_qf:
                td_train_op = tf.compat.v1.train.AdamOptimizer(self._qf_lr).minimize(
        ...
        # with tf.GradientTape(persistent=False) as tape_q:
        ...
        with tf.compat.v1.variable_scope('target_qf_opt_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
            if self._train_qf:
                ind_q_train_op = tf.compat.v1.train.AdamOptimizer(self._qf_lr).minimize(
    ...
    def _create_p_update(self):
        ...
        # with tf.compat.v1.variable_scope('target_p_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
        ...
        if self._k <= 1:
            ...
            self_actions, all_actions = self.policy.actions_for(
                observations=self._observations_ph,
                reuse=tf.compat.v1.AUTO_REUSE, all_action=True)
            assert_shape(self_actions, [None, self._action_dim])
        ...
        q_targets -= tf.math.log(tf.cast(self._value_n_particles, tf.float32))
        ...
        if self._aux:
            ...
            if self._k > 1:
                q_k= self.joint_qf.output_for(
                    ...
                    opponent_actions=all_actions[-2], reuse=tf.compat.v1.AUTO_REUSE)
            ...
            if self._k > 3:
                ...
                q_k = self.joint_qf.output_for(
                    observations=self._next_observations_ph,
                    actions=all_actions[-3],
                    opponent_actions=all_actions[-4], reuse=tf.compat.v1.AUTO_REUSE)
                q_k_2 = self.joint_qf.output_for(
                    observations=self._next_observations_ph,
        ...
        self._pg_loss = tf.stop_gradient(pg_loss)
        # todo add level k Q loss:


        with tf.compat.v1.variable_scope('policy_opt_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
            if self._train_policy:
                optimizer = tf.compat.v1.train.AdamOptimizer(self._policy_lr)
    ...
    def _create_conditional_policy_svgd_update(self):
        ...
        with tf.GradientTape(persistent=False) as tape:
            ...
            # To that end, we first sample a single set of
        ...
        with tf.compat.v1.variable_scope('conditional_policy_opt_agent_{}'.format(self._agent_id), reuse=tf.compat.v1.AUTO_REUSE):
            if self._train_policy:
                optimizer = tf.compat.v1.train.AdamOptimizer(self._policy_lr)
    ...
    def _get_feed_dict(self, batch, annealing=1.):
    ...
    def ret_q_mean(self, batch):
        """Returns diagnostic information.
        Returns the mean and standard deviation of Q-function
        for a sample batch.
        """

        feeds = self._get_feed_dict(batch)
        qf = self._sess.run(tf.stop_gradient(self._pg_loss), feeds)
